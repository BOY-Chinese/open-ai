...
| 页面 | 能力 |
|---|---|
| 账号 | 四通道账号增删、启用开关、**真状态监控**（🟢 正常 / 🔴 断连 / 🟠 服务未响应）、Chromium 登录助手、刷新积分、补签、重连 |
| Auto 路由链 | 多链管理、编辑器（增删/排序/逐模型超时）、链检查与单模型检查、模型简报 |
| 模型列表 | 四通道模型与积分倍率（三态：真 0 显示 0.00、未知显示 `--`）、显隐/置顶、**右键「检查该模型」**（向上游发极短探测请求，右下角 Toast 报告结果，不加状态列） |
| API 管理 | 密钥创建/重命名/删除，创建时间持久化 |
| 积分 | 今日/本周积分看板（本地流水库聚合，毫秒级） |
| 日志 | 操作日志（记录用户在界面上的每个动作与结果，含登录脚本输出实时跟随） |
| 设置 | 开机自启、一键更新、一键卸载 |
