...
│          │  │                                │ │
│  ─────── │  └────────────────────────────────┘ │
│  操作日志 │  [添加 Trae] [添加 WorkBuddy] [...]   │ ← 底部固定栏
│  系统设置 │                                      │
...
- **本地缓存（v3.0.1）**：进页面不再空等
  ...
  | 问题 | 做法 |
  |---|---|
  | 积分倍率要逐个通道联网查询（秒级），列表本体只是内存读取（毫秒级） | 把「三通道全量模型 + 已叠加视图偏好」的结果落一份到 `localStorage`（`lib/modelCache.ts`，键 `open-ai.models-cache.v1`） |
  | 每次进页面都先看到骨架屏 | 首帧**同步**读缓存直接渲染（`useState(readModelCache)`），后台再拉最新数据覆盖；期间工具栏显示「本地缓存」徽章，失败则显示「离线 · 本地缓存」并如实说明数据是旧的 |
  | 切换通道 / 显隐筛选各触发一次秒级请求 | 改为**只拉一次全量**，筛选由 `filterModelView()` 在本地完成 —— 筛选瞬间生效，不再有第二次骨架屏 |

  三个取舍：**缓存全量而非按筛选分片**（一份缓存覆盖所有筛选组合）；**同步读写**
  （异步读会闪一帧骨架屏，等于没优化）；**过期不丢弃**（超 30 分钟只把 `stale` 标为 true
  提示可刷新，陈旧列表也比空屏有用）。缓存由页面在 `status === 'success'` 时统一写入 ——
  否则「首帧用的是缓存」会把缓存原样回写并刷新 `savedAt`，用旧数据冒充新数据。
...
- 图例、Tooltip、柱子取自**同一次** `seriesColors(useTheme().resolved)` 调用，三者不会脱钩
  （v3.0.1 起配色随主题切换，详见 4.7；此前是从 `getComputedStyle` 读 CSS 变量，
  但图表色本身并不在 CSS 变量里，那处说明与实现不符，已一并纠正）

### 4.5 操作日志（v3.0.1 由「系统日志」改回 v2.3 语义）

**语义纠正**：v3.0 初版这一页读的是 `logs/gateway_out.log`，展示的是**网关自己在说什么**
（上游请求、路由、异常）。v2.3 的「操作日志」记的是**用户在界面上做了什么**——
点了刷新、加了账号、改了密钥、重连了通道，以及每次操作的结果与耗时。两者服务的问题
不同：网关日志面向排障，操作日志回答「我刚才那一下到底成没成」，后者才是界面用户
真正会问的问题。故改回操作日志。

记账格式沿用 v2.3 `gui_account_manager._run()` / `_write_log()` 的习惯：

```
2026/09/13 01:08  ────────────────────────────────────────────────────────
2026/09/13 01:08  [操作] 添加账号
2026/09/13 01:08  [参数] 通道=Trae；已拉起 login_trae.py，请在浏览器完成登录，输出见 logs/login_Trae.log
2026/09/13 01:08  [完成] 01:08:44（耗时 652ms）          ← success 绿色
```

- **埋点在数据源层**（`lib/oplogBackend.ts` 的 Proxy 包在 `backend` 外），不是页面里逐处
  `log(...)`。写操作有十几处，逐处手写必漏；代理则「要么忘了在 `MUTATIONS` 里登记而被
  开发期告警挡住，要么一登记就自动获得完整记账」。只读方法刻意不记（页面一打开就会调它们，
  记进去只会用噪声淹掉真正的操作）。
- **`[完成]` 一定落笔**（`try/finally` 语义）：否则日志里会出现「一个操作开始了但没有下文」，
  读者无法判断是还在跑还是崩了。这与 v2.3 的 `_thread_wrap` 一致。
- **本地保存**（`localStorage`，上限 2000 行，超限从头部丢弃）。v2.3 只存内存，
  关掉 GUI 就没了；而「我十分钟前那次添加账号报了错」恰恰是重启后才想起来要看的。
- 沿用已验证的日志查看器体验：`font-mono` + `bg-bg-log` + 级别着色、
  **自动滚动 + 智能暂停**（距底部 > 40px 即停，浮出「回到最新」）、
  **Ctrl/Cmd+F** 行内搜索、文本区 `select-text`（全局 `user-select: none` 的唯一例外）。
- 级别筛选从四级收敛为「全部 / 成功 / 警告 / 失败 / 过程」：「只看失败」是排障时最常用的动作。
- 顶部「日志目录」按钮只在 Tauri 内出现（浏览器预览里 `invoke` 不可用），
  网关自身的运行输出仍从那里查看 —— 操作日志不该把排障入口一起带走。

#### 4.5.1 登录脚本输出实时跟随（v2.3 `_subprocess_stream` 的等价物）

v2.3 的 `_subprocess_stream()` 用 `Popen(stdout=PIPE)` 逐行读登录脚本输出并塞进日志队列，
所以「添加账号」那一段会实时滚出 `login_trae.py` 的进度。v3.0 的界面与网关是两个进程、
脚本由网关拉起，故用**增量轮询**等价实现：

```
POST /v1/admin/accounts/login   →  {logOffset: <启动时的文件长度>}
GET  /v1/admin/accounts/login/log?channel=&offset=   →  {text, offset, running, exists, exitCode}
```

- **`logOffset` 是关键**：`logs/login_<通道>.log` 是**追加**写的，历次登录的输出都在里面。
  不带起始偏移，用户会看到上一次登录的旧输出被当成这一次的。
- **`running` 靠 Popen 句柄判断**，不是猜文件 mtime —— 用户把浏览器晾在一边十分钟时，
  mtime 推断必然误判。网关重启会丢句柄（此时返回 `running=false`，前端停止跟随，
  已读到的输出不受影响）。
- 前端在 `lib/loginStream.ts` 里每秒拉一段，按行贴进日志，并按内容给级别着色
  （判据偏保守：**不**单凭「完成」二字判成功 —— 实测踩到「等待用户在浏览器中完成登录 ...」
  被染成绿色，而那一行的意思恰恰是还没完成）。
- **两次收尾**：脚本结束 → 写 `[提示] 登录成功后新账号即可参与轮询` 并结束跟随 →
  通过 `lib/accountEvents` 通知账号列表页自动刷新（v2.3 的 `refresh_account_list()` 等价物）。
  没有这一步，用户必须自己想起「点一下刷新账号」才能看到新账号。
- **一个操作块**：`addAccount` 会**等登录脚本结束才 resolve**，这样代理写下的
  `[完成]` 落在脚本输出**之后**。若在这里 fire-and-forget 立刻返回，`[完成]` 会插在
  输出前面，读起来变成「操作已完成，然后（莫名其妙地）又有一堆输出」。
  代价是 `addAccount` 可能几分钟才返回，**因此调用方刻意不 await 它** ——
  用户可能在浏览器里填两分钟密码，界面不该锁在那儿。按钮改为进入「登录中…」并禁用
  （防连点：连点会顶掉网关侧的 Popen 句柄，前一次跟随就再也判断不出「还在跑吗」）。
...
- 启动设置卡片内新增**外观设置**入口（见 4.7）

### 4.7 外观设置（白天 / 夜间 / 跟随系统）

入口在 **系统设置 → 启动设置 → 「外观设置」按钮**，点开是一个三项单选对话框。

| 模式 | 行为 | 说明文案 |
|---|---|---|
| 白天（默认） | 始终浅色 | 始终使用浅色配色（默认） |
| 夜间 | 始终深色 | 始终使用深色配色 |
| 跟随系统 | 按 `prefers-color-scheme` 实时切换 | 随 Windows 的浅色 / 深色设置自动切换（附「当前系统：深色/浅色」） |

**实现要点**

- **令牌分两套**：`:root` = 浅色，`.dark` = 深色。两者特异性相同（都是 0,1,0），
  故**顺序即优先级**，`.dark` 必须写在 `:root` 之后。切换方式是给 `<html>` 加/去 `dark`。
- **默认白天**，不是「跟随系统」——后者在深色系统的机器上首次打开就是深色，与需求相悖。
- **防白闪**：首帧的 `dark` class 由 `index.html` 的**内联脚本**写好。等 React 挂载后再切，
  用户会先看到一帧默认主题再跳过去（深色系统上就是白闪）。该脚本与 `lib/theme.ts`
  是同一份约定的两个副本，改一处必须同步另一处（两处都写了这条注释）。
- **浅色对比度逐项核算**（不是估的）：正文 17.9:1 / 次要 6.0:1 / 弱化 4.8:1 / 极弱 3.4:1；
  语义色按「同色 12% 底上 ≥4.5:1」选定（深色主题那套亮绿 #22C55E、亮黄 #EAB308
  在白底上分别只有 2.1:1 / 1.9:1，直接复用会不可读）。
- **图表配色随主题换**：recharts 的 `fill` 不走 CSS 变量，是唯一「主题换了颜色不一定跟着换」
  的地方，故 `config/chart.ts` 提供深浅两套色板，由 `useTheme()` 选择。
  亮色套色相不变、整体压暗（#6B7280 / #1A9E4A / #BD6705，对白底 4.8 / 3.5 / 4.1:1）。
- **用原生 `<input type="radio">`**（`sr-only` + `peer` 焦点环），方向键切换 / Tab 焦点 /
  屏幕阅读器播报全部白送；自绘按钮组要重新实现一遍还容易漏。
- **点击即生效，无「保存」按钮**：主题是所见即所得的东西，选完再确认纯属多余。
  （对比：卸载那种不可逆操作才需要二次确认。）
- 「跟随系统」模式下**才**订阅 `matchMedia('change')`：用户显式选了白天/夜间时不该被
  系统主题变化打断（否则表现为「我明明选了夜间，插个显示器自己变白了」）

> **已知限制**：`src-tauri/tauri.conf.json` 原先写死 `"theme": "Dark"`，会**强制 WebView2
> 报告 `prefers-color-scheme: dark`**，使「跟随系统」永远只能得到深色。已移除该项
> （`backgroundColor` 同步改为浅色底，与默认的白天模式一致）。
> 代价：标题栏跟随**系统**主题，而不是用户选的外观模式 —— 用户在浅色系统下选「夜间」时，
> 标题栏仍是浅色。要让标题栏也跟随，需要 `core:window:allow-set-theme` 权限
> （Tauri 2 需显式 capabilities，本项目当前没有 capabilities 目录）并在 Rust 侧重建；
> 权衡后未做，避免为一个装饰性差异引入无法在本机验证的 Rust 构建改动。
...
### 5.2 「每日签到」列的语义（v3.0.1 已按需求纠正）

~~需求列名为「每日签到」，但原始截图中该列呈现为勾选框且语义不明。**当前实现**：可勾选的 Checkbox（本地状态），语义为"该账号今日是否已签到"。~~

**纠正后**：该列是**今日是否签到成功的只读状态**，不是「是否要每日签到」的开关。

原实现有两处硬伤：

1. **渲染成可勾选的 Checkbox** —— 勾选只写进一个纯本地的 `useState`，既不落库、
   也不触发任何签到动作。给用户一个能点但没有作用的控件，比不给更糟。
2. **初值取 `a.enabled`**（`signin[a.id] ?? a.enabled`）—— 把「账号是否启用」当成了
   「今日是否签到」。这两个量毫无关系，属于会误导判断的错误数据。

**现在的数据来源**：`GET /v1/admin/accounts/signin`（新增，只读、不联网）→
`data/usage_history.db` 的 `gain` 表。**当日有入账记录即签到成功**，这是客观凭证：

- TRAE 写 `kind='checkin'`；
- WorkBuddy 系写当日入账的资源包（`usage_collector.collect_gains` 明确注释：
  WB 的 `checkin-status` 在 `active=false` 的账号上恒报未签，只有资源包入账才可信）。

渲染为 `已签到`（success Badge）/ `未签到`（outline Badge），悬停显示入账积分、
凭证类型与时间。

**边界处理**：接口不可用时显示 **「—」**（未知）而不是「未签到」。签到表里
「没这个账号」在当前实现里就等于「未签到」，不区分的话，网关没重启（新接口还没上线）
会被渲染成「今天所有账号都没签到」—— 一个看起来完全正常的错误结论。
（此路径已实测：用未重启的网关访问，五列全部显示「—」。）
...
**顺带修复**：`open-ai-autostart.bat` 原本硬编码 `D:\…\open-ai` 这类绝对路径，
...
页面组件 ──> @/lib/dataSource ──┬─> httpBackend  (真实网关 /v1/admin/*)  默认
                                └─> mockBackend  (纯前端演示数据)      ?mock=1
                                        ↓ 外面统一包一层
                                 withOpLog(…)  操作日志埋点代理（v3.0.1）
```

两种实现共用同一套函数签名，因此 `AccountsPage` 等页面完全不需要知道数据来自哪里。

**为什么 `backend` 是 Proxy 而不是原对象**：v3.0.1 起所有写操作自动记入操作日志
（见 4.5）。埋点放在这一层而不是页面里，是为了让「新增写操作」这件事**不可能静默漏记**。
衍生影响：判定演示模式不能再写 `backend === mockBackend`（Proxy ≠ 原对象），
改用未包装的 `rawSource` 比较 —— 这类「包了一层之后等值判断失效」的坑在
`dataSource.ts` 里已就地注释，避免后人再踩。
...
| 页面 | 已验证要点 |
|---|---|
| 账号管理 | 三通道合一表格、5 列完整、**每日签到只读状态（已签到/未签到/— 三分支）**、积分明细、状态 Badge、底部三按钮、行悬停高亮 |
| API 管理 | 网关卡片（**无复制按钮**）、密钥掩码、右键菜单提示、底部操作栏 |
| 模型列表 | 通道色点、倍率单位（×/credits）、路由模型名、置顶 Badge、多选模式入口、**缓存徽章 + 二次进入首帧即有数据（0 骨架屏）** |
| 积分看板（今日） | 双统计卡片（绿/红）、流水表含「所属通道」列、级别筛选 |
| 积分看板（每周） | **三通道堆叠柱状图配色正确且随主题切换**（浅色 #6B7280/#1A9E4A/#BD6705 ↔ 深色 #868E96/#22C55E/#EAB308）、图例色块、周切换器、Tooltip |
| 操作日志 | 等宽字体、级别着色、`────` 分隔与 `[操作]/[参数]/[完成]` 结构、工具栏、条数统计、搜索计数 |
| 系统设置 | 三卡片顺序（启动/版本/危险操作）、开关、**外观设置入口与三选一对话框**、危险操作红框 |

外观与操作日志另有专项自检工具 `tools/verify-appearance.mjs`（27 条断言）：
默认主题=白天、三模式切换与持久化、跟随系统实时响应 `prefers-color-scheme` 翻转、
图表配色随主题切换、操作日志埋点落笔、模型缓存三通道全覆盖、每日签到无勾选框。

### 交付前检查清单（对照 ui-ux-pro-max 规范）

- [x] 无 emoji 结构图标（全部 lucide-react 矢量图标）
- [x] 可交互元素均有 hover + active 反馈 + `cursor-pointer`
- [x] **深浅两套主题的对比度都独立核算达标**（不是从一套推断另一套）：
      深色 正文 19.2:1 / 次要 7.4:1 / 图形 3.4:1；浅色 正文 17.9:1 / 次要 6.0:1 /
      图形 3.5:1；两套的语义色在同色 12% 底上均 ≥4.5:1
- [x] 语义令牌，组件内零硬编码 hex（图表色集中一处并注明原因；
      滚动条 / 骨架微光 / 阴影 / 遮罩 / 开关轨道也全部令牌化，否则浅色下会出现
      「白色微光在白底上不可见」这类只在单侧主题暴露的问题）
...
1. **依赖必须装在本机盘**
   ...
   ```bash
   ln -sfn <WSL本地盘路径>/open-ai/node_modules_store/node_modules \
           <项目根>/desktop-ui/node_modules
   ```
...
| 后续清理 | ① 这一步**没有重建 exe** —— `open-ai-launcher.exe` 打包于 09-02，而 `launcher_main.py` 09-12 才修，所以那份 exe 里仍是「写死拉起已删除的 tkinter GUI」的旧代码。所幸 ② 已把快捷方式改指桌面端，这个漏洞一直没暴露。<br>现已把 `open-ai-launcher.exe`、`launcher_internal/`、`launcher_main.py`、`launcher_version.txt` **一并删除**（master 确认不再需要；安装包链路后续重建）。<br>实测确认：本机启动链（快捷方式 / 开机自启 / `bootstrap.py`）**无一处引用它**，删后运行无任何影响。 |
...
- `launcher_main.py`：删掉 Python GUI 兜底，桌面端缺失时**明确弹窗报错**
  （该文件与其打包产物 `open-ai-launcher.exe` + `launcher_internal/` 后续已整体删除 ——
  见 12.1 的后续说明）
